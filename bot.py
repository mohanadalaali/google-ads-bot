...
def notify(alerts: List[Alert], total_risk: int) -> None:
    ...
    lines = [
        "🛡️ <b>Google Ads Guard Alert</b>",
        f"Risk Score: <b>{total_risk}</b>",
        ""
    ]

    for alert in alerts[:20]:
        lines.append(f"{alert.level} <b>{alert.title}</b> (+{alert.risk})")
        lines.append(alert.body[:900])
        lines.append("")
...
def check_ads(client, customer_id: str) -> List[Alert]:
    ...
    if disapproved_count > int(env("DISAPPROVED_LIMIT", "2")):
        alerts.append(Alert(
            "🛑",
            "عدد إعلانات مرفوضة عالي",
            f"Disapproved ads: {disapproved_count}",
            40
        ))
    ...
    if limited_count > int(env("APPROVED_LIMITED_LIMIT", "5")):
        alerts.append(Alert(
            "⚠️",
            "عدد إعلانات محدودة عالي",
            f"Limited ads: {limited_count}",
            25
        ))
...
def check_campaign_costs(client, customer_id: str) -> List[Alert]:
    ga_service = client.get_service("GoogleAdsService")
    alerts: List[Alert] = []
...
def pause_enabled_campaigns(client, customer_id: str) -> None:
    ga_service = client.get_service("GoogleAdsService")
...
def main() -> None:
    ...
    if not customer_id:
        send_telegram("⚠️ Google Ads غير مربوط بعد\n✅ Telegram شغال والبوت يعمل بنجاح.")
        return

    if not GOOGLE_ADS_ENABLED:
        send_telegram("⚠️ مكتبة Google Ads غير مثبتة\nتأكد أن google-ads موجودة في requirements.txt")
        return

    client = build_google_ads_client()

    if client is None:
        send_telegram("⚠️ لم يتم إنشاء Google Ads Client")
        return
    ...
    total_risk = sum(alert.risk for alert in alerts)
...
if __name__ == "__main__":
    try:
        ...
        send_telegram("❌ Google Ads API Error:\n" + str(ex)[:3500])

    except Exception as ex:
        logging.exception("Bot failed")
        send_telegram("❌ Bot failed:\n" + str(ex)[:3500])
